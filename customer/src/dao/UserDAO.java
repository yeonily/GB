...
//   아이디 중복검사   
   public int checkId(String userId) {
      ...
      try {
         ...
         if(resultSet.next()) {
            code = DUPLICATED_ID_CODE; //중복된 번호
            
            if(resultSet.getInt(1) == DELETED_USER_CODE) { //탈퇴된 번호와 동일하다면
               code = DELETED_USER_CODE; //코드에는 탈퇴된 번호
               
            }
         }else {
            code = ENABLED_ID_CODE; //그 외 사용 가능한 번호
         }
